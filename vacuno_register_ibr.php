...
<title>Vacuno - Registro de Vacunación IBR</title>
<!-- Link to the Favicon -->
            ...
            <div class="row justify-content-center align-items-stretch">
                ...
                <div class="col-md-4 d-flex px-1 mb-3 mb-md-0">
                    <div class="arrow-step arrow-step-active w-100">
                        <span class="badge-active">🎯 Registrando Vacuna IBR</span>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="newEntryModalLabel">
                    <i class="fas fa-plus-circle me-2"></i>Nueva Vacunacion Ibr
                </h5>
                    ...
                    <div class="mb-4">                        
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <label for="new_vacuna" class="form-label">Vacunas Ibr</label>
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    <option value="">Vacunas</option>
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT vc_ibr_vacuna FROM vc_ibr ORDER BY vc_ibr_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['vc_ibr_vacuna']) . '">' . htmlspecialchars($vacuna_row['vc_ibr_vacuna']) . '</option>';
                                        }
              ...
              <tbody>
                  ...
                  try {
                      $ibrQuery = "SELECT 
                                          ...
                                          v.nombre,
                                          va.vh_ibr_fecha,
                                          va.id AS ibr_record_id, -- Alias ibr record ID
                                          va.vh_ibr_producto,
                                          va.vh_ibr_dosis,
                                          va.vh_ibr_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.vh_ibr_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_ibr_history
                                      FROM 
                                          vacuno v
                                      LEFT JOIN 
                                          -- Join with all ibr records per tagid
                                          vh_ibr va ON v.tagid = va.vh_ibr_tagid 
                                      ORDER BY 
                                          va.vh_ibr_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($ibrsData)) {
                          ...
                          try {
                              ...
                              if ($row_config && isset($row_config['v_vencimiento_ibr'])) {
                                  $vigencia = intval($row_config['v_vencimiento_ibr']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($ibrsData as $row) {
                              ...
                              if ($row['in_ibr_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['vh_ibr_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['vh_ibr_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['vh_ibr_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['vh_ibr_fecha'])) {
                              ...
                              echo '        <button class="btn btn-success register-new-ibr-btn" style="height: 30px !important; width: 30px !important; padding: 0 !important; font-size: 1rem !important; line-height: 30px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important;"
                              ...
                              if ($row['in_ibr_history'] == 1) {
                                  echo '        <button class="btn btn-warning edit-ibr" style="height: 30px !important; width: 30px !important; padding: 0 !important; font-size: 1rem !important; line-height: 30px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important;"
                                                  data-id="' . htmlspecialchars($row['ibr_record_id'] ?? '') . '" 
                                                  data-tagid="' . htmlspecialchars($row['tagid'] ?? '') . '" 
                                                  data-vacuna="' . htmlspecialchars($row['vh_ibr_producto'] ?? '') . '" 
                                  ...
                                  echo '        <button class="btn btn-danger delete-ibr" style="height: 30px !important; width: 30px !important; padding: 0 !important; font-size: 1rem !important; line-height: 30px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important;"
                                                  data-id="' . htmlspecialchars($row['ibr_record_id'] ?? '') . '"
...
$(document).ready(function() {
    ...
    // Note: editIbrModal is created dynamically later, so no need to initialize here.
    ...
    $('.edit-ibr').click(function() {
        ...
        // Edit Ibr Modal dialog for editing
        var modalHtml = `
        <div class="modal fade" id="editIbrModal" tabindex="-1" aria-labelledby="editIbrModalLabel">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editIbrModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacunacion Ibr
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editIbrForm">
                            ...
                            <div class="mb-4">                        
                                <div class="input-group">
                                    <span class="input-group-text">
                                        <i class="fa-solid fa-syringe"></i>
                                        <label for="edit_vacuna" class="form-label">Vacunas Ibr</label>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="vacunaSelectionModalLabel">
                            <i class="fas fa-list me-2"></i>Seleccionar Vacuna de IBR
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['vc_ibr_vacuna']) . '">' . htmlspecialchars($vacuna_row['vc_ibr_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
    ...
    $(document).on('click', '.register-new-ibr-btn', function() { 
...
<!-- Professional CSS Styling for IBR Charts -->
<style>
...
.card:hover .card-header {
    background: linear-gradient(135deg, #e83e8c 0%, #6f42c1 100%) !important;
}
...
$(document).ready(function() {
    ...
    function createIbrChart(data) {
        ...
        lineGradient.addColorStop(0, 'rgba(111, 66, 193, 1)');
        lineGradient.addColorStop(1, 'rgba(232, 62, 140, 1)');

        const fillGradient = ctx.createLinearGradient(0, 0, 0, 400);
        fillGradient.addColorStop(0, 'rgba(111, 66, 193, 0.3)');
        fillGradient.addColorStop(0.5, 'rgba(111, 66, 193, 0.1)');
        fillGradient.addColorStop(1, 'rgba(111, 66, 193, 0.05)');
                ...
                animation: {
                    ...
                    onProgress: function(animation) {
                        ...
                        if (meta.data.length > 0) {
                            ...
                            if (lastPoint) {
                                ctx.save();
                                ctx.shadowColor = 'rgba(111, 66, 193, 0.6)';
                                ctx.shadowBlur = 25;
                                ctx.shadowOffsetX = 0;
                                ctx.shadowOffsetY = 0;
                                ctx.restore();
...
<!-- Pie Chart Section -->
<div class="container mt-5 mb-5">
    <h4 class="text-center text-black mb-4">Estado de Vacunación Ibr por Grupos</h4>
    <div class="row justify-content-center" id="groupChartsContainer">
        <!-- Group charts will be populated dynamically here -->
    </div>
</div>

<!-- Chart.js Script for Ibr Status Pie Charts -->
<script>
$(document).ready(function() {
    const ibrStatusUrl = 'get_ibr_status_by_grupo.php';
                ...
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        <div class="chart-container" style="position: relative; height:40vh; width:100%">
                            <canvas id="ibrPie${safeGrupo}"></canvas>
                            <div id="ibrPie${safeGrupo}Message" class="text-center small text-muted mt-2"></div>
                        </div>
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.error) {
                console.error('Error fetching ibr status data:', data.error);
            ...
            grupos.forEach(grupo => {
                ...
                const canvasId = `ibrPie${safeGrupo}`;
                const messageId = `ibrPie${safeGrupo}Message`;
        ...
        error: function(xhr, status, error) {
            console.error('AJAX error fetching ibr status data:', error);
