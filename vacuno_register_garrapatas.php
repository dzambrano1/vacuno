...
<title>Vacuno - Registro de Vacunación Garrapatas</title>
<!-- Link to the Favicon -->
            ...
            <div class="row justify-content-center align-items-stretch">
                ...
                <div class="col-md-4 d-flex px-1 mb-3 mb-md-0">
                    <div class="arrow-step arrow-step-active w-100">
                        <span class="badge-active">🎯 Registrando Tratamientos Garrapatas</span>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="newEntryModalLabel">
                    <i class="fas fa-plus-circle me-2"></i>Nueva Vacunacion Garrapatas
                </h5>
                    ...
                    <div class="mb-4">                        
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <label for="new_vacuna" class="form-label">Vacunas Garrapatas</label>
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    <option value="">Vacunas</option>
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT vc_garrapatas_vacuna FROM vc_garrapatas ORDER BY vc_garrapatas_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['vc_garrapatas_vacuna']) . '">' . htmlspecialchars($vacuna_row['vc_garrapatas_vacuna']) . '</option>';
                                        }
      ...
      <div class="table-responsive">
          <table id="garrapatasTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                  try {
                      $garrapatasQuery = "SELECT 
                                          v.id AS vacuno_id, -- Alias vacuno ID
                                          v.tagid,
                                          v.nombre,
                                          va.vh_garrapatas_fecha,
                                          va.id AS garrapatas_record_id, -- Alias garrapatas record ID
                                          va.vh_garrapatas_producto,
                                          va.vh_garrapatas_dosis,
                                          va.vh_garrapatas_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.vh_garrapatas_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_garrapatas_history
                                      FROM 
                                          vacuno v
                                      LEFT JOIN 
                                          -- Join with all garrapatas records per tagid
                                          vh_garrapatas va ON v.tagid = va.vh_garrapatas_tagid 
                                      ORDER BY 
                                          va.vh_garrapatas_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($garrapatassData)) {
                          ...
                          try {
                              ...
                              if ($row_config && isset($row_config['v_vencimiento_garrapatas'])) {
                                  $vigencia = intval($row_config['v_vencimiento_garrapatas']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($garrapatassData as $row) {
                           ...
                           echo "<td>" . htmlspecialchars($row['vh_garrapatas_fecha'] ?? '') . "</td>";
                              ...
                              if ($row['in_garrapatas_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['vh_garrapatas_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['vh_garrapatas_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['vh_garrapatas_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['vh_garrapatas_fecha'])) {
                              ...
                              echo '        <button class="btn btn-success register-new-garrapatas-btn" style="height: 30px !important; width: 30px !important; padding: 0 !important; font-size: 1rem !important; line-height: 30px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important;"
                              ...
                              if ($row['in_garrapatas_history'] == 1) {
                                  echo '        <button class="btn btn-warning edit-garrapatas" style="height: 30px !important; width: 30px !important; padding: 0 !important; font-size: 1rem !important; line-height: 30px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important;"
                                                  data-id="' . htmlspecialchars($row['garrapatas_record_id'] ?? '') . '" 
                                  ...
                                  echo '        <button class="btn btn-danger delete-garrapatas" style="height: 30px !important; width: 30px !important; padding: 0 !important; font-size: 1rem !important; line-height: 30px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important;"
                                                  data-id="' . htmlspecialchars($row['garrapatas_record_id'] ?? '') . '"
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editGarrapatasModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacunacion Garrapatas
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editGarrapatasForm">
                            ...
                            <div class="mb-4">                        
                                <div class="input-group">
                                    <span class="input-group-text">
                                        <i class="fa-solid fa-syringe"></i>
                                        <label for="edit_vacuna" class="form-label">Vacunas Garrapatas</label>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="vacunaSelectionModalLabel">
                            <i class="fas fa-list me-2"></i>Seleccionar Vacuna de Garrapatas
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['vc_garrapatas_vacuna']) . '">' . htmlspecialchars($vacuna_row['vc_garrapatas_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
...
<!-- Professional CSS Styling for Garrapatas Charts -->
<style>
...
<!-- Chart.js Script for Garrapatas Line Chart -->
<script>
$(document).ready(function() {
    let allGarrapatasData = [];
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data.error) {
                ...
                $('#garrapatasChart').parent().html('<div class="alert alert-danger">Error: ' + data.error + '</div>');
                return;
            }
            
            if (!data || data.length === 0) {
                console.log('No data available');
                $('#garrapatasChart').parent().html('<div class="alert alert-info">No hay datos de garrapatas disponibles para mostrar en el gráfico.</div>');
                return;
            }
            
            allGarrapatasData = data;
        ...
        error: function(xhr, status, error) {
            ...
            $('#garrapatasChart').parent().html('<div class="alert alert-danger">Error al cargar datos: ' + error + '</div>');
        }
    });
    ...
    function updateChart() {
        ...
        let filteredData = [...allGarrapatasData];
    ...
    function createGarrapatasChart(data) {
        ...
        fillGradient.addColorStop(0, 'rgba(220, 53, 69, 0.3)');
        fillGradient.addColorStop(0.5, 'rgba(220, 53, 69, 0.1)');
        fillGradient.addColorStop(1, 'rgba(220, 53, 69, 0.05)');
...
<!-- Pie Chart Section -->
<div class="container mt-5 mb-5">
    <h4 class="text-center text-black mb-4">Estado de Vacunación Garrapatas por Grupos</h4>
    <div class="row justify-content-center" id="groupChartsContainer">
        <!-- Group charts will be populated dynamically here -->
    </div>
</div>

<!-- Chart.js Script for Garrapatas Status Pie Charts -->
<script>
$(document).ready(function() {
    const garrapatasStatusUrl = 'get_garrapatas_status_by_grupo.php';
                ...
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        <div class="chart-container" style="position: relative; height:40vh; width:100%">
                            <canvas id="garrapatasPie${safeGrupo}"></canvas>
                            <div id="garrapatasPie${safeGrupo}Message" class="text-center small text-muted mt-2"></div>
                        </div>
    ...
    $.ajax({
        url: garrapatasStatusUrl,
        type: 'GET',
        dataType: 'json',
        success: function(data) {
            if (data.error) {
                console.error('Error fetching garrapatas status data:', data.error);
            ...
            grupos.forEach(grupo => {
                ...
                const canvasId = `garrapatasPie${safeGrupo}`;
                const messageId = `garrapatasPie${safeGrupo}Message`;
        ...
        error: function(xhr, status, error) {
            console.error('AJAX error fetching garrapatas status data:', error);
