...
<title>Registros Pesaje Leche</title>
<!-- Link to the Favicon -->
...
<div class="container scroll-icons-container">
    <button class="btn btn-outline-secondary mb-3" type="button" onclick="window.location.href='./vacuno_registrar_pesaje_animal.php'" data-tooltip="Pesaje Carne">
        <img src="./images/carne.png" alt="Pesaje Carne" class="nav-icon">
    </button>
    
    <button class="icon-button" type="button" data-tooltip="Pesaje Leche" data-bs-target="#">
        <img src="./images/leche-cantaro.png" alt="Pesaje Leche" class="nav-icon">
    </button>
    
    <button class="btn btn-outline-secondary mb-3" type="button" onclick="window.location.href='./vacuno_registrar_alimentacion.php'" data-tooltip="Alimentacion">
    ...
    <button class="btn btn-outline-secondary mb-3" type="button" onclick="window.location.href='./vacuno_registrar_reproduccion.php'" data-tooltip="Reproducion">
        <img src="./images/el-embarazo.png" alt="Reproduccion" class="nav-icon">
    </button>

    <button class="btn btn-outline-secondary mb-3" type="button" data-bs-toggle="collapse" onclick="window.location.href='./vacuno_registrar_poblacion.php'" data-tooltip="Poblacion">
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM vh_leche WHERE vh_leche_tagid = ? ORDER BY vh_leche_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM vh_leche ORDER BY vh_leche_fecha ASC");
        $stmt->execute();
    }
    ...
    // Process leche data
    $lecheData = $stmt->fetchAll(); // Fetch all records at once
    foreach ($lecheData as $row) {
        ...
        $monthlyWeights[$monthKey][] = floatval($row['vh_leche_peso']);
        $lecheFechaLabels[] = $row['vh_leche_fecha'];
    }

    // Initialize and calculate monthly data
    $monthlyData = array_fill(0, count($lecheFechaLabels), null);
    ...
    $monthlyPrices = [];
    $monthlyPriceData = array_fill(0, count($lecheFechaLabels), null);
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['vh_leche_precio']);
    ...
    $stmt->execute(); // Re-execute to reset the cursor
    $monthlyValues = [];
    $monthlyValueData = array_fill(0, count($lecheFechaLabels), null);
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['vh_leche_peso']) * floatval($row['vh_leche_precio']);
...
<?php
    ...
    // Fetch all leche records with animal name and total value calculation
    $lecheQuery = "SELECT p.*, 
    ...
    $stmt = $conn->prepare($lecheQuery);
    $stmt->execute();
    $lechesResult = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
$(document).ready(function() {
    $('#lecheForm').on('submit', function(e) {
        e.preventDefault();
                ...
                .then(data => {
                    if (data.success) {
                        Swal.fire({
                            ...
                            window.location.href = data.redirect || 'vacuno_registrar_leche.php';
                        });
                    } else {
...
<div class="table-responsive">
  <!-- Add New Leche Form -->
  <button class="btn btn-success mb-3 text-center" type="button" data-bs-toggle="collapse" data-bs-target="#addLecheForm">
  <i class="fas fa-plus"></i> Registrar
  </button>
  <table id="lechesTable" class="table table-striped table-bordered">
      <thead>
          <tr>
...
<?php
// Display tables
foreach ($lechesResult as $row) {
    if ($row['vh_leche_tagid'] == '3000') {
        error_log("Debug - TagID 3000:");
        error_log("leche_animal: " . $row['vh_leche_peso']);
        error_log("leche_precio: " . $row['vh_leche_precio']);
    ...
    echo "<td class='text-center'>" . htmlspecialchars($row['vh_leche_precio']) . "</td>";
    ...
    echo "<td class='text-center'>
            ...
            <button class='btn btn-success btn-sm edit-leche'
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.delete-leche').forEach(button => {
        ...
        <div class="modal-content">
            ...
            <form id="editLecheForm" action="process_leche.php" method="POST">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label class="form-label">Leche (kg)</label>
                        <input type="number" step="0.01" class="form-control" name="leche" id="edit_leche" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Precio por kg ($)</label>
...
<div class="chart-container">
    <div class="card h-100">
        <div class="card-body">
            <h5 class="card-title">Evolución Producción Lactea</h5>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const labels = <?php echo json_encode($lecheFechaLabels); ?>;
    ...
    const weightChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: 'Evolución Lactea (Tendencia)',
                    font: {
                        size: 16
                    }
                },
