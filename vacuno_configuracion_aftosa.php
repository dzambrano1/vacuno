...
<title>Vacuno Configuracion Aftosa</title>
<!-- Link to the Favicon -->
        ...
        <div class="col-12 col-md-6 col-lg-4">
            <div class="d-flex justify-content-center">
                <div class="arrow-step arrow-step-active w-100" style="border-radius: 15px; clip-path: none;">
                    <span class="badge-active">🎯 Estás configurando Aftosa</span>
...
<!-- New Entry Modal Configuracion Aftosa -->

<!-- Add New Vacuna Aftosa Button -->
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nueva Vacuna Aftosa
              </h5>
          ...
          <table id="aftosaTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
                    ...
                    <th class="text-center" style="width: 10%;">Vigencia (dias)</th>
                  </tr>
              </thead>
              <tbody>
                  <?php
                      $aftosaQuery = "SELECT * FROM vc_aftosa";
                      ...
                      if (empty($aftosasData)) {
                          ...
                          foreach ($aftosasData as $row) {
                              ...
                              echo '        <button class="btn btn-warning edit-aftosa" style="height: 40px !important; width: 40px !important; padding: 0 !important; font-size: 1rem !important; line-height: 30px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important;"
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '" 
                                              data-vacuna="' . htmlspecialchars($row['vc_aftosa_vacuna'] ?? '') . '" 
                                              data-dosis="' . htmlspecialchars($row['vc_aftosa_dosis'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['vc_aftosa_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['vc_aftosa_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna Aftosa">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger delete-aftosa" style="height: 40px !important; width: 40px !important; padding: 0 !important; font-size: 1rem !important; line-height: 30px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important;"
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              title="Eliminar Configuracion Vacuna Aftosa">
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['vc_aftosa_vacuna'] ?? '') . "</td>";
                              
                              // Column 2: Dosis
                              echo "<td>" . htmlspecialchars($row['vc_aftosa_dosis'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td>" . htmlspecialchars($row['vc_aftosa_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td>" . htmlspecialchars($row['vc_aftosa_vigencia'] ?? 'N/A') . "</td>";
...
$(document).ready(function() {
    ...
    $('#saveNewAftosa').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar la vacuna ${formData.vacuna} con dosis de ${formData.dosis} ml?`,
            icon: 'question',
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_aftosa.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
    ...
    $('.edit-aftosa').click(function() {
        ...
        // Edit Configuracion Aftosa Modal dialog for editing
        var modalHtml = `
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editAftosaModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacuna Aftosa
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
        ...
        $('#saveEditAftosa').click(function() {
            ...
            var form = document.getElementById('editAftosaForm');
            if (!form.checkValidity()) {
                form.reportValidity();
                return;
            }
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de aftosa?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-aftosa').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Está seguro de que desea eliminar la configuracion de aftosa? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
