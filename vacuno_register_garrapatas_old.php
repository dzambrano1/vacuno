...
<title>Vacuno Register Garrapatas</title>
<!-- Link to the Favicon -->
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="newEntryModalLabel">
                    <i class="fas fa-plus-circle me-2"></i>Nuevo Tratamiento Garrapatas
                </h5>
                  ...
                  try {
                      $garrapatasQuery = "SELECT 
                                      ...
                                      FROM 
                                          vacuno v
                                      LEFT JOIN 
                                          -- Join with the latest garrapatas record per tagid
                                          vh_garrapatas va ON v.tagid = va.vh_garrapatas_tagid 
                                          AND va.id = (
                                              SELECT MAX(va_inner.id) 
                                              FROM vh_garrapatas va_inner 
                                              WHERE va_inner.vh_garrapatas_tagid = v.tagid
                                          )
                                      ORDER BY 
                                          in_garrapatas_history DESC, v.nombre ASC;";
                      ...
                      if (empty($garrapatassData)) {
                          ...
                          foreach ($garrapatassData as $row) {
                              ...
                              echo '        <button class="btn btn-success btn-sm add-garrapatas-btn" 
...
$(document).ready(function() {
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editGarrapatasModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Tratamiento Garrapatas
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
    ...
    newEntryModalEl.addEventListener('show.bs.modal', function (event) {
        ...
        var modalForm = newEntryModalEl.querySelector('#newGarrapatasForm'); // Correct form ID
    ...
    // Removed the old .register-new-garrapatas click listener
});
</script>
...
<div class="container mt-5 mb-5">
    <div class="card shadow">
        <div class="card-header bg-success text-white">
            <h5 class="mb-0"><i class="fas fa-chart-line me-2"></i>Evolución Dosis Garrapatas</h5>
        </div>
        <div class="card-body">
            <div class="row mb-4">
                <div class="col-md-4">
                    ...
                    title: {
                        ...
                        text: function() {
                            ...
                            if (selectedAnimal !== 'all') {
                                ...
                                return 'Evolución de Dosis de Garrapatas - ' + animalName;
                            }
                            return 'Evolución de Dosis de Garrapatas - Todos los Animales';
                        },
                        font: {
                            size: 16
                        }
...
<!-- Pie Chart Section -->
<div class="container mt-5 mb-5">
    <h4 class="text-center text-black mb-4">Estado de Vacunación Garrapatas por Etapa</h4>
    <div class="row justify-content-center">
            ...
            <div class="card shadow">
                ...
                <div class="card-body">
                    <div class="chart-container" style="position: relative; height:40vh; width:100%">
                        <canvas id="garrapatasPieInicio"></canvas>
                        <div id="garrapatasPieInicioMessage" class="text-center small text-muted mt-2"></div>
                    </div>
        ...
        <div class="col-md-4 mb-4">
            <div class="card shadow">
                ...
                <div class="card-body">
                    <div class="chart-container" style="position: relative; height:40vh; width:100%">
                        <canvas id="garrapatasPieCrecimiento"></canvas>
                        <div id="garrapatasPieCrecimientoMessage" class="text-center small text-muted mt-2"></div>
                    </div>
                     <!-- Toggle Button and Table for Non-Vaccinated -->
                    <div class="text-center mt-2">
        ...
        <div class="col-md-4 mb-4">
            <div class="card shadow">
                ...
                <div class="card-body">
                    <div class="chart-container" style="position: relative; height:40vh; width:100%">
                        <canvas id="garrapatasPieFinalizacion"></canvas>
                         <div id="garrapatasPieFinalizacionMessage" class="text-center small text-muted mt-2"></div>
                    </div>
                     <!-- Toggle Button and Table for Non-Vaccinated -->
                     <div class="text-center mt-2">
...
<script>
$(document).ready(function() {
    const garrapatasStatusUrl = 'get_garrapatas_status_by_etapa.php';
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.error) {
                console.error('Error fetching garrapatas status data:', data.error);
                 $('#garrapatasPieInicioMessage, #garrapatasPieCrecimientoMessage, #garrapatasPieFinalizacionMessage').text('Error al cargar datos.');
                return;
            }
            ...
            stages.forEach(stage => {
                 ...
                 const canvasId = `garrapatasPie${stage}`;
                 const messageId = `garrapatasPie${stage}Message`;
        ...
        error: function(xhr, status, error) {
            ...
            $('#garrapatasPieInicioMessage, #garrapatasPieCrecimientoMessage, #garrapatasPieFinalizacionMessage').text('Error de conexión al cargar datos.');
        }
    });
