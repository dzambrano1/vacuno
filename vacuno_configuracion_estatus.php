...
<title>Vacuno Configuracion Estatus</title>
<!-- Link to the Favicon -->
        ...
        <div class="col-12 col-md-6 col-lg-4">
            <div class="d-flex justify-content-center">
                <div class="arrow-step arrow-step-active w-100" style="border-radius: 15px; clip-path: none;">
                    <span class="badge-active">🎯 Estás configurando Estatus</span>
...
<div class="modal fade" id="newEntryModal" tabindex="-1" aria-labelledby="newEntryModalLabel">
  ...
  <!-- DataTable for vc_estatus records -->
...
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="estatusTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $estatusQuery = "SELECT * FROM vc_estatus";
                      ...
                      if (empty($estatussData)) {
                          ...
                          foreach ($estatussData as $row) {
                              ...
                              echo '        <button class="btn btn-danger delete-estatus" style="height: 40px !important; width: 40px !important; padding: 0 !important; font-size: 1rem !important; line-height: 30px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important;"
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              data-estatus="' . htmlspecialchars($row['vc_estatus_nombre'] ?? '') . '"
                              ...
                              // Column 1: Estatus
                              echo "<td>" . htmlspecialchars($row['vc_estatus_nombre'] ?? '') . "</td>";
...
<!-- Initialize DataTable for Vacuno Estatus -->
<script>
                                ...
                                <div class="input-group">
                                    <span class="input-group-text">
                                        ...
                                        <select class="form-select" id="edit_estatus" name="estatus" required>
                                            ...
                                            $sql_names = "SELECT DISTINCT vc_estatus_nombre FROM vc_estatus ORDER BY vc_estatus_nombre ASC";
                                            $stmt_names = $conn->prepare($sql_names);
                                            $stmt_names->execute();
                                            $names = $stmt_names->fetchAll(PDO::FETCH_ASSOC);
                                            foreach ($names as $name_row) {
                                                echo '<option value="' . htmlspecialchars($name_row['vc_estatus_nombre']) . '">' . htmlspecialchars($name_row['vc_estatus_nombre']) . '</option>';
                                            }
                                            ?>
                                        </select>
                                    </span>                                    
