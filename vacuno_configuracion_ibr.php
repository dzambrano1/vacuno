...
<title>Vacuno Configuracion Ibr</title>
<!-- Link to the Favicon -->
        ...
        <div class="col-12 col-md-6 col-lg-4">
            <div class="d-flex justify-content-center">
                <div class="arrow-step arrow-step-active w-100" style="border-radius: 15px; clip-path: none;">
                    <span class="badge-active">🎯 Estás configurando Ibr</span>
...
<!-- New Entry Modal Configuracion Ibr -->

<!-- Add New Vacuna Ibr Button -->
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nueva Vacuna Ibr
              </h5>
                  ...
                  <?php
                      ...
                      if (empty($ibrsData)) {
                          ...
                          foreach ($ibrsData as $row) {
                              ...
                              echo '        <button class="btn btn-warning edit-ibr" style="height: 40px !important; width: 40px !important; padding: 0 !important; font-size: 1rem !important; line-height: 30px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important;"
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '" 
                                              data-vacuna="' . htmlspecialchars($row['vc_ibr_vacuna'] ?? '') . '" 
                                              data-dosis="' . htmlspecialchars($row['vc_ibr_dosis'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['vc_ibr_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['vc_ibr_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna Ibr">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger delete-ibr" style="height: 40px !important; width: 40px !important; padding: 0 !important; font-size: 1rem !important; line-height: 30px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important;"
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              title="Eliminar Configuracion Vacuna Ibr">
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['vc_ibr_vacuna'] ?? '') . "</td>";
                              
                              // Column 2: Dosis
                              echo "<td>" . htmlspecialchars($row['vc_ibr_dosis'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td>" . htmlspecialchars($row['vc_ibr_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td>" . htmlspecialchars($row['vc_ibr_vigencia'] ?? 'N/A') . "</td>";
...
$(document).ready(function() {
    ...
    $('#saveNewIbr').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar la dosis de ibr ${formData.dosis} ml ?`,
            icon: 'question',
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_ibr.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
    ...
    $('.edit-ibr').click(function() {
        ...
        // Edit Configuracion Ibr Modal dialog for editing
        var modalHtml = `
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editIbrModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacuna Ibr
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
        ...
        $('#saveEditIbr').click(function() {
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de ibr?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-ibr').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Está seguro de que desea eliminar la configuracion de ibr? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
