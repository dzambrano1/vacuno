...
try {
    ...
    $sql = "
        SELECT
            ...
            vh_melaza_costo
        FROM vh_melaza
        WHERE vh_melaza_racion > 0 AND vh_melaza_costo > 0 -- Consider only valid cost entries
        ORDER BY vh_melaza_tagid, vh_melaza_fecha ASC;
    ";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            $tagid = $row['vh_melaza_tagid'];
            $fecha = $row['vh_melaza_fecha'];
            $daily_cost = (float)$row['vh_melaza_racion'] * (float)$row['vh_melaza_costo'];
