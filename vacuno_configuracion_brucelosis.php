...
<title>Vacuno Configuracion Brucelosis</title>
<!-- Link to the Favicon -->
        ...
        <div class="col-12 col-md-6 col-lg-4">
            <div class="d-flex justify-content-center">
                <div class="arrow-step arrow-step-active w-100" style="border-radius: 15px; clip-path: none;">
                    <span class="badge-active">🎯 Estás configurando Brucelosis</span>
...
<!-- New Entry Modal Configuracion Brucelosis -->

<!-- Add New Vacuna Brucelosis Button -->
<div class="container my-3 text-center">
  <button type="button" class="btn btn-success text-center" data-bs-toggle="modal" data-bs-target="#newEntryModal">
    <i class="fas fa-plus-circle me-2"></i>Nueva Vacuna Brucelosis
  </button>
</div>
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nueva Vacuna Brucelosis
              </h5>
...
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="brucelosisTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $brucelosisQuery = "SELECT * FROM vc_brucelosis";

                      $stmt = $conn->prepare($brucelosisQuery);
                      $stmt->execute();
                      $brucelosissData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($brucelosissData)) {
                          echo "<tr><td colspan='5' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          foreach ($brucelosissData as $row) {
                              echo "<tr>";
                              
                              // Column 0: Actions
                              echo '<td class="text-center">';
                              echo '    <div class="btn-group" role="group">';
                              echo '        <button class="btn btn-warning edit-brucelosis" style="height: 40px !important; width: 40px !important; padding: 0 !important; font-size: 1rem !important; line-height: 30px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important;"
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '" 
                                              data-vacuna="' . htmlspecialchars($row['vc_brucelosis_vacuna'] ?? '') . '" 
                                              data-dosis="' . htmlspecialchars($row['vc_brucelosis_dosis'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['vc_brucelosis_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['vc_brucelosis_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna Brucelosis">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger delete-brucelosis" style="height: 40px !important; width: 40px !important; padding: 0 !important; font-size: 1rem !important; line-height: 30px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important;"
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              title="Eliminar Configuracion Vacuna Brucelosis">
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['vc_brucelosis_vacuna'] ?? '') . "</td>";
                              
                              // Column 2: Dosis
                              echo "<td>" . htmlspecialchars($row['vc_brucelosis_dosis'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td>" . htmlspecialchars($row['vc_brucelosis_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td>" . htmlspecialchars($row['vc_brucelosis_vigencia'] ?? 'N/A') . "</td>";
...
$(document).ready(function() {
    ...
    $('#saveNewBrucelosis').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar la dosis de brucelosis ${formData.dosis} ml ?`,
            icon: 'question',
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_brucelosis.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
    ...
    $('.edit-brucelosis').click(function() {
        ...
        // Edit Configuracion Brucelosis Modal dialog for editing
        var modalHtml = `
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editBrucelosisModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacuna Brucelosis
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
        ...
        $('#saveEditBrucelosis').click(function() {
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de brucelosis?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-brucelosis').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Está seguro de que desea eliminar la configuracion de brucelosis? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
