...
<title>Vacuno Register Feed</title>
<!-- Link to the Favicon -->
...
<div class="container scroll-icons-container">
    <button class="btn btn-outline-secondary mb-3" type="button" onclick="window.location.href='./vacuno_register_herd.php'" class="icon-button" data-tooltip="POBLACION">
...
<div class="container text-center">
  ...
  <div class="container mb-4" style="display:block; justify-content: center; align-items: center;">
      <h4 class="container text-center">ALIMENTO CONCENTRADO</h4>
  </div>
          ...
          <div class="modal-content">
              <div class="modal-header">
                  <h5 class="modal-title" id="newEntryModalLabel">
                      <i class="fas fa-plus-circle me-2"></i>Nuevo Registro Alimento Concentrado
                  </h5>
              ...
              <div class="modal-body">
                  <form id="newFeedForm">
                      ...
                      <div class="mb-4">
                          <label for="new_feed" class="form-label">Concentrado (kg)</label>
                          <div class="input-group">
                              <span class="input-group-text"><i class="fas fa-balance-scale"></i></span>
                              <input type="number" step="0.01" class="form-control" id="new_feed" name="feed" required>
                          </div>
                      </div>
                      <div class="mb-4">
              ...
              <div class="modal-footer">
                  ...
                  <button type="button" class="btn btn-success" id="saveNewFeed">
  ...
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="vh_feedTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
                      ...
                      <th class="text-center">Concentrado (kg)</th>
              ...
              <tbody>
                  <?php
                  try {
                      // Query to get all vh_concentrado records with animal name
                      $feedQuery = "SELECT c.*, v.nombre as animal_nombre,
                                CAST((c.vh_concentrado_racion * c.vh_concentrado_costo) AS DECIMAL(10,2)) as total_value
                                FROM vh_concentrado c 
                                LEFT JOIN vacuno v ON c.vh_concentrado_tagid = v.tagid 
                                ORDER BY c.vh_concentrado_fecha DESC";
                                
                      $stmt = $conn->prepare($feedQuery);
                      $stmt->execute();
                      $feedsData = $stmt->fetchAll(PDO::FETCH_ASSOC);
                      
                      // If no data, display a message
                      if (empty($feedsData)) {
                          echo "<tr><td colspan='9' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          // Get vigencia setting for feed records
                          ...
                          foreach ($feedsData as $row) {
                              ...
                              try {
                                  $feedDate = new DateTime($row['vh_concentrado_fecha']);
                                  $dueDate = clone $feedDate;
                                  ...
                                  error_log("Date error: " . $e->getMessage() . " for date: " . $row['vh_concentrado_fecha']);
                              ...
                              echo '<td class="text-center">
                                  <div class="btn-group" role="group">
                                      <button class="btn btn-warning btn-sm edit-feed" 
                                          data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                          data-tagid="' . htmlspecialchars($row['vh_concentrado_tagid'] ?? '') . '"
                                          data-feed="' . htmlspecialchars($row['vh_concentrado_racion'] ?? '') . '"
                                          data-precio="' . htmlspecialchars($row['vh_concentrado_costo'] ?? '') . '"
                                          data-fecha="' . htmlspecialchars($row['vh_concentrado_fecha'] ?? '') . '">
                                          <i class="fas fa-edit"></i>
                                      </button>
                                      <button class="btn btn-danger btn-sm delete-feed" 
                                          data-id="' . htmlspecialchars($row['id'] ?? '') . '">
                      ...
                      error_log("Error in feed table: " . $e->getMessage());
...
$(document).ready(function() {
    ...
    $('#saveNewFeed').click(function() {
        // Validate the form
        var form = document.getElementById('newFeedForm');
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar el pesaje dela leche ${formData.feed} kg para el animal con Tag ID ${formData.tagid}?`,
            icon: 'question',
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de feed ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-feed').click(function() {
        ...
        var peso = $(this).data('feed');
        var precio = $(this).data('precio');
        var fecha = $(this).data('fecha');
        
        // Edit Feed Modal dialog for editing
        ...
        <div class="modal fade" id="editFeedModal" tabindex="-1" aria-labelledby="editFeedModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editFeedModalLabel">
                    ...
                    <div class="modal-body">
                        <form id="editFeedForm">
                            ...
                            <div class="mb-4">
                                <label for="edit_feed" class="form-label">Leche (kg)</label>
                                <div class="input-group">
                                    <span class="input-group-text"><i class="fas fa-balance-scale"></i></span>
                                    <input type="number" step="0.01" class="form-control" id="edit_feed" value="${feed}" required>
                                </div>
                            </div>
                            <div class="mb-4">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-success" id="saveEditFeed">
        ...
        $('#editFeedModal').remove();
        
        // Add the modal to the page
        $('body').append(modalHtml);
        
        // Show the modal
        var editModal = new bootstrap.Modal(document.getElementById('editFeedModal'));
        editModal.show();
        
        // Handle save button click
        $('#saveEditFeed').click(function() {
            var formData = {
                id: $('#edit_id').val(),
...
<!-- Feed Line Chart Section -->

<div class="container mt-5 mb-5">
    <div class="card shadow">
        <div class="card-header bg-success text-white">
            <h5 class="mb-0"><i class="fas fa-chart-line me-2"></i>Evolución Alimento Concentrado</h5>
        </div>
        <div class="card-body">
            <div class="row mb-4">
                <div class="col-md-4">
...
<!-- Chart.js Script for Feed Line Chart -->
<script>
$(document).ready(function() {
    let allFeedData = [];
    let feedChart = null;
    
    // Fetch feed data and create the chart
    $.ajax({
