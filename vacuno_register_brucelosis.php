...
<title>Vacuno - Registro de Vacunación Brucelosis</title>
<!-- Link to the Favicon -->
            ...
            <div class="row justify-content-center align-items-stretch">
                ...
                <div class="col-md-4 d-flex px-1 mb-3 mb-md-0">
                    <div class="arrow-step arrow-step-active w-100">
                        <span class="badge-active">🎯 Registrando Vacuna Brucelosis</span>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="newEntryModalLabel">
                    <i class="fas fa-plus-circle me-2"></i>Nueva Vacunacion Brucelosis
                </h5>
                    ...
                    <div class="mb-4">                        
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <label for="new_vacuna" class="form-label">Vacunas Brucelosis</label>
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    <option value="">Vacunas</option>
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT vc_brucelosis_vacuna FROM vc_brucelosis ORDER BY vc_brucelosis_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['vc_brucelosis_vacuna']) . '">' . htmlspecialchars($vacuna_row['vc_brucelosis_vacuna']) . '</option>';
                                        }
      ...
      <div class="table-responsive">
          <table id="brucelosisTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                  try {
                      $brucelosisQuery = "SELECT 
                                          v.id AS vacuno_id, -- Alias vacuno ID
                                          v.tagid,
                                          v.nombre,
                                          va.vh_brucelosis_fecha,
                                          va.id AS brucelosis_record_id, -- Alias brucelosis record ID
                                          va.vh_brucelosis_producto,
                                          va.vh_brucelosis_dosis,
                                          va.vh_brucelosis_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.vh_brucelosis_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_brucelosis_history
                                      FROM 
                                          vacuno v
                                      LEFT JOIN 
                                          -- Join with all brucelosis records per tagid
                                          vh_brucelosis va ON v.tagid = va.vh_brucelosis_tagid 
                                      ORDER BY 
                                          va.vh_brucelosis_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($brucelosisData)) {
                          ...
                          try {
                              ...
                              if ($row_config && isset($row_config['v_vencimiento_brucelosis'])) {
                                  $vigencia = intval($row_config['v_vencimiento_brucelosis']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($brucelosisData as $row) {
                              ...
                              if ($row['in_brucelosis_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['vh_brucelosis_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['vh_brucelosis_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['vh_brucelosis_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['vh_brucelosis_fecha'])) {
                              ...
                              echo '        <button class="btn btn-success register-new-brucelosis-btn" style="height: 30px !important; width: 30px !important; padding: 0 !important; font-size: 1rem !important; line-height: 30px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important;"
                              ...
                              if ($row['in_brucelosis_history'] == 1) {
                                  echo '        <button class="btn btn-warning edit-brucelosis" style="height: 30px !important; width: 30px !important; padding: 0 !important; font-size: 1rem !important; line-height: 30px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important;"
                                                  data-id="' . htmlspecialchars($row['brucelosis_record_id'] ?? '') . '" 
                                  ...
                                  echo '        <button class="btn btn-danger delete-brucelosis" style="height: 30px !important; width: 30px !important; padding: 0 !important; font-size: 1rem !important; line-height: 30px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important;"
                                                  data-id="' . htmlspecialchars($row['brucelosis_record_id'] ?? '') . '"
...
$(document).ready(function() {
    ...
    // Note: editbrucelosisModal is created dynamically later, so no need to initialize here.
    ...
    $('.edit-brucelosis').click(function() {
        ...
        var modalHtml = `
        <div class="modal fade" id="editBrucelosisModal" tabindex="-1" aria-labelledby="editBrucelosisModalLabel">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editBrucelosisModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacunacion Brucelosis
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editBrucelosisForm">
                            ...
                            <div class="mb-4">                        
                                <div class="input-group">
                                    <span class="input-group-text">
                                        <i class="fa-solid fa-syringe"></i>
                                        <label for="edit_vacuna" class="form-label">Vacunas Brucelosis</label>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="vacunaSelectionModalLabel">
                            <i class="fas fa-list me-2"></i>Seleccionar Vacuna de Brucelosis
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['vc_brucelosis_vacuna']) . '">' . htmlspecialchars($vacuna_row['vc_brucelosis_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
    ...
    $(document).on('click', '.register-new-brucelosis-btn', function() { 
...
<!-- Professional CSS Styling for Brucelosis Charts -->
<style>
...
$(document).ready(function() {
    ...
    function createBrucelosisChart(data) {
        ...
        lineGradient.addColorStop(0, 'rgba(102, 126, 234, 1)');
        lineGradient.addColorStop(1, 'rgba(118, 75, 162, 1)');

        const fillGradient = ctx.createLinearGradient(0, 0, 0, 400);
        fillGradient.addColorStop(0, 'rgba(102, 126, 234, 0.3)');
        fillGradient.addColorStop(0.5, 'rgba(102, 126, 234, 0.1)');
...
<!-- Pie Chart Section -->
<div class="container mt-5 mb-5">
    <h4 class="text-center text-black mb-4">Estado de Vacunación Brucelosis por Grupos</h4>
...
<script>
$(document).ready(function() {
    const brucelosisStatusUrl = 'get_brucelosis_status_by_grupo.php';
                ...
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        <div class="chart-container" style="position: relative; height:40vh; width:100%">
                            <canvas id="brucelosisPie${safeGrupo}"></canvas>
                            <div id="brucelosisPie${safeGrupo}Message" class="text-center small text-muted mt-2"></div>
                        </div>
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            grupos.forEach(grupo => {
                ...
                const canvasId = `brucelosisPie${safeGrupo}`;
                const messageId = `brucelosisPie${safeGrupo}Message`;
