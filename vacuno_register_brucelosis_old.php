...
<title>Vacuno Register Brucelosis</title>
<!-- Link to the Favicon -->
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    <option value="">Brucelosis</option>
                                    <?php
                                    try {
                  ...
                  try {
                      $brucelosisQuery = "SELECT 
                                      ...
                                      FROM 
                                          vacuno v
                                      LEFT JOIN 
                                          -- Join with the latest brucelosis record per tagid
                                          vh_brucelosis va ON v.tagid = va.vh_brucelosis_tagid 
                                          AND va.id = (
                                              SELECT MAX(va_inner.id) 
                                              FROM vh_brucelosis va_inner 
                                              WHERE va_inner.vh_brucelosis_tagid = v.tagid
                                          )
                                      ORDER BY 
                                          in_brucelosis_history DESC, v.nombre ASC;";
                      ...
                      if (empty($brucelosissData)) {
                          ...
                          foreach ($brucelosissData as $row) {
                              ...
                              echo '        <button class="btn btn-success btn-sm add-brucelosis-btn" 
                              ...
                              if ($row['in_brucelosis_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-brucelosis" 
                                  ...
                                  echo '        <button class="btn btn-danger btn-sm delete-brucelosis" 
                                                  data-id="' . htmlspecialchars($row['brucelosis_record_id'] ?? '') . '"
...
$(document).ready(function() {
    $('#brucelosisTable').DataTable({
        ...
        // Order by Estatus column descending
        order: [[7, 'desc']],
        ...
        columnDefs: [
            {
                targets: [0], // Actions column is now the first
                orderable: false,
                searchable: false
            },
            {
                targets: [5, 6], // Dosis, Costo columns indices shifted
            ...
            {
                 targets: [1], // Fecha Nac column index shifted
                render: function(data, type, row) {
                    if (type === 'display') {
...
$(document).ready(function() {
    ...
    newEntryModalEl.addEventListener('show.bs.modal', function (event) {
        ...
        var modalForm = newEntryModalEl.querySelector('#newBrucelosisForm');
...
<div class="container mt-5 mb-5">
    <div class="card shadow">
        <div class="card-header bg-success text-white">
            <h5 class="mb-0"><i class="fas fa-chart-line me-2"></i>Evolución Dosis Brucelosis</h5>
        </div>
        <div class="card-body">
            <div class="row mb-4">
                <div class="col-md-4">
