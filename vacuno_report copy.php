...
$sql_weight = "SELECT vh_peso_fecha, vh_peso_animal, vh_peso_precio FROM vh_peso WHERE vh_peso_tagid = ? ORDER BY vh_peso_fecha DESC";
...
if ($result_weight->num_rows > 0) {
    ...
    while ($row = $result_weight->fetch_assoc()) {
        $data[] = array($row['vh_peso_fecha'], $row['vh_peso_animal'], $row['vh_peso_precio']);
    }
    $pdf->SimpleTable($header, $data);
