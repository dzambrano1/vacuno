...
<title>Vacuno Configuracion Sal</title>
<!-- Link to the Favicon -->
                  ...
                  <?php
                      ...
                      if (empty($salsData)) {
                          ...
                          foreach ($salsData as $row) {
                              ...
                              // Column 0: Producto
                              echo "<td>" . htmlspecialchars($row['vc_sal_nombre'] ?? '') . "</td>";
                              // Columna 1: Etapa
                              echo "<td>" . htmlspecialchars($row['vc_sal_etapa'] ?? '') . "</td>";
                              
                              // Column 2: Racion
                              echo "<td>" . htmlspecialchars($row['vc_sal_racion'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td>" . htmlspecialchars($row['vc_sal_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td>" . htmlspecialchars($row['vc_sal_vigencia'] ?? 'N/A') . "</td>";

                              // Column 5: Actions
                              echo '<td class="text-center">';
                              echo '    <div class="btn-group" role="group">';
                              echo '        <button class="btn btn-warning edit-sal" style="height: 40px !important; width: 40px !important; padding: 0 !important; font-size: 1rem !important; line-height: 30px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important;"
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '" 
                                              data-sal="' . htmlspecialchars($row['vc_sal_nombre'] ?? '') . '" 
                                              data-etapa="' . htmlspecialchars($row['vc_sal_etapa'] ?? '') . '" 
                                              data-racion="' . htmlspecialchars($row['vc_sal_racion'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['vc_sal_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['vc_sal_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna Sal">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger delete-sal" style="height: 40px !important; width: 40px !important; padding: 0 !important; font-size: 1rem !important; line-height: 30px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important;"
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                        ...
                        <form id="editSalForm">
                            ...
                            <div class="mb-4">
                                ...
                                <select class="form-select" id="edit_sal" name="sal" required>
                                    ...
                                    $sql_names = "SELECT DISTINCT vc_sal_nombre FROM vc_sal ORDER BY vc_sal_nombre ASC";
                                    $stmt_names = $conn->prepare($sql_names);
                                    $stmt_names->execute();
                                    $names = $stmt_names->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($names as $name_row) {
                                        echo '<option value="' . htmlspecialchars($name_row['vc_sal_nombre']) . '">' . htmlspecialchars($name_row['vc_sal_nombre']) . '</option>';
                                    }
                                    ?>
                                </select>
                            </div>
