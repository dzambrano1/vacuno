...
<title>Vacuno Configuracion Cbr</title>
<!-- Link to the Favicon -->
        ...
        <div class="col-12 col-md-6 col-lg-4">
            <div class="d-flex justify-content-center">
                <div class="arrow-step arrow-step-active w-100" style="border-radius: 15px; clip-path: none;">
                    <span class="badge-active">🎯 Estás configurando CBR</span>
...
<!-- New Entry Modal Configuracion Cbr -->

<!-- Add New Vacuna Cbr Button -->
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nueva Vacuna Cbr
              </h5>
                  ...
                  <?php
                      ...
                      if (empty($cbrsData)) {
                          ...
                          foreach ($cbrsData as $row) {
                              ...
                              echo '        <button class="btn btn-warning edit-cbr" style="height: 40px !important; width: 40px !important; padding: 0 !important; font-size: 1rem !important; line-height: 30px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important;"
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '" 
                                              data-vacuna="' . htmlspecialchars($row['vc_cbr_vacuna'] ?? '') . '" 
                                              data-dosis="' . htmlspecialchars($row['vc_cbr_dosis'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['vc_cbr_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['vc_cbr_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna Cbr">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger delete-cbr" style="height: 40px !important; width: 40px !important; padding: 0 !important; font-size: 1rem !important; line-height: 30px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important;"
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              title="Eliminar Configuracion Vacuna Cbr">
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['vc_cbr_vacuna'] ?? '') . "</td>";
                              
                              // Column 2: Dosis
                              echo "<td>" . htmlspecialchars($row['vc_cbr_dosis'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td>" . htmlspecialchars($row['vc_cbr_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td>" . htmlspecialchars($row['vc_cbr_vigencia'] ?? 'N/A') . "</td>";
...
$(document).ready(function() {
    ...
    $('#saveNewCbr').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar la dosis de cbr ${formData.dosis} ml ?`,
            icon: 'question',
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_cbr.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
    ...
    $('.edit-cbr').click(function() {
        ...
        // Edit Configuracion Cbr Modal dialog for editing
        var modalHtml = `
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editCbrModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacuna Cbr
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
        ...
        $('#saveEditCbr').click(function() {
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de cbr?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-cbr').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Está seguro de que desea eliminar la configuracion de cbr? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
