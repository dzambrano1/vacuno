...
try {
    ...
    // Get the validity period for sal from configuration
    ...
    if ($configResult && mysqli_num_rows($configResult) > 0) {
        ...
        $validityDays = intval($configRow['v_vencimiento_sal']);
    }

    // SQL query to get individual sal records for proper cross-month distribution
    $sql = "SELECT 
                vh_sal_racion,
                vh_sal_costo,
                vh_sal_fecha_inicio
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $startDate = new DateTime($row['vh_sal_fecha_inicio']);
            ...
            $totalExpense = (float)$row['vh_sal_racion'] * (float)$row['vh_sal_costo'] * $totalDays;
            ...
            while ($currentDate <= $endDate) {
                ...
                $monthlyData[$monthKey]['total_quantity'] += (float)$row['vh_sal_racion'];
                $monthlyData[$monthKey]['total_cost_sum'] += (float)$row['vh_sal_costo'];
    ...
    error_log("Error in get_sal_expense_data.php: " . $e->getMessage());
    
    // Return error response
    http_response_code(500);
    echo json_encode(array(
        'error' => 'Error al obtener datos de gastos en sal',
