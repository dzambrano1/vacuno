...
try {
    ...
    // SQL query to get individual concentrado records with actual date ranges
    $sql = "SELECT 
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            // Get actual date range from database columns
            $startDate = new DateTime($row['vh_concentrado_fecha_inicio']);
            $endDate = new DateTime($row['vh_concentrado_fecha_fin']);
            ...
            $totalExpense = (float)$row['vh_concentrado_racion'] * (float)$row['vh_concentrado_costo'] * $totalDays;
            ...
            while ($currentDate <= $endDate) {
                ...
                if (!isset($monthlyData[$monthKey])) {
                    $monthlyData[$monthKey] = array(
                        'total_expense' => 0,
                        'total_quantity' => 0,
                        'total_cost_sum' => 0, // For average price calculation
                        'record_count' => 0,
                        'days_count' => 0
                    );
                }
                ...
                $monthlyData[$monthKey]['total_quantity'] += (float)$row['vh_concentrado_racion'];
                $monthlyData[$monthKey]['total_cost_sum'] += (float)$row['vh_concentrado_costo'];
            ...
            // Count unique records (avoid counting the same record multiple times for record_count)
    ...
    foreach ($monthlyData as $month => $monthData) {
        ...
        $data[] = array(
            ...
            'days_count' => $monthData['days_count'] // Additional info for debugging
        );
    }
    ...
    echo json_encode(array(
        'error' => 'Error al obtener datos de gastos en concentrado',
