...
try {
    // 1. Fetch all relevant concentrado records, ordered correctly
    $sql = "
        SELECT
        ...
        FROM vh_concentrado
        WHERE vh_concentrado_racion > 0 AND vh_concentrado_costo > 0 -- Consider only valid cost entries
    ...
    if (!$result) {
        throw new Exception("Error fetching concentrado data: " . mysqli_error($conn));
    }
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $daily_cost = (float)$row['vh_concentrado_racion'] * (float)$row['vh_concentrado_costo'];
