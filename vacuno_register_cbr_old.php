                  ...
                  try {
                      $cbrQuery = "SELECT 
                                      ...
                                      FROM 
                                          vacuno v
                                      LEFT JOIN 
                                          -- Join with the latest cbr record per tagid
                                          vh_cbr va ON v.tagid = va.vh_cbr_tagid 
                                          AND va.id = (
                                              SELECT MAX(va_inner.id) 
                                              FROM vh_cbr va_inner 
                                              WHERE va_inner.vh_cbr_tagid = v.tagid
                                          )
                                      ORDER BY 
                                          in_cbr_history DESC, v.nombre ASC;";
                      ...
                      if (empty($cbrsData)) {
                          ...
                          foreach ($cbrsData as $row) {
                              ...
                              echo '        <button class="btn btn-success btn-sm add-cbr-btn" 
                              ...
                              if ($row['in_cbr_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-cbr" 
                                  ...
                                  echo '        <button class="btn btn-danger btn-sm delete-cbr" 
                                                  data-id="' . htmlspecialchars($row['cbr_record_id'] ?? '') . '"
...
$(document).ready(function() {
    ...
    newEntryModalEl.addEventListener('show.bs.modal', function (event) {
        ...
        var modalForm = newEntryModalEl.querySelector('#newCBRForm'); // Updated form ID reference
...
<div class="container mt-5 mb-5">
    <div class="card shadow">
        <div class="card-header bg-success text-white">
            <h5 class="mb-0"><i class="fas fa-chart-line me-2"></i>Evolución Dosis CBR</h5>
        </div>
        <div class="card-body">
            <div class="row mb-4">
                <div class="col-md-4">
...
<!-- Pie Chart Section -->
<div class="container mt-5 mb-5">
    <h4 class="text-center text-black mb-4">Estado de Vacunación CBR por Etapa</h4>
    <div class="row justify-content-center">
            ...
            <div class="card shadow">
                ...
                <div class="card-body">
                    <div class="chart-container" style="position: relative; height:40vh; width:100%">
                        <canvas id="cbrPieInicio"></canvas>
                        <div id="cbrPieInicioMessage" class="text-center small text-muted mt-2"></div>
                    </div>
        ...
        <div class="col-md-4 mb-4">
            <div class="card shadow">
                ...
                <div class="card-body">
                    <div class="chart-container" style="position: relative; height:40vh; width:100%">
                        <canvas id="cbrPieCrecimiento"></canvas>
                        <div id="cbrPieCrecimientoMessage" class="text-center small text-muted mt-2"></div>
                    </div>
                     <!-- Toggle Button and Table for Non-Vaccinated -->
                    <div class="text-center mt-2">
        ...
        <div class="col-md-4 mb-4">
            <div class="card shadow">
                ...
                <div class="card-body">
                    <div class="chart-container" style="position: relative; height:40vh; width:100%">
                        <canvas id="cbrPieFinalizacion"></canvas>
                         <div id="cbrPieFinalizacionMessage" class="text-center small text-muted mt-2"></div>
                    </div>
                     <!-- Toggle Button and Table for Non-Vaccinated -->
                     <div class="text-center mt-2">
...
<script>
$(document).ready(function() {
    const cbrStatusUrl = 'get_cbr_status_by_etapa.php';
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.error) {
                console.error('Error fetching cbr status data:', data.error);
                 $('#cbrPieInicioMessage, #cbrPieCrecimientoMessage, #cbrPieFinalizacionMessage').text('Error al cargar datos.');
                return;
            }
            ...
            stages.forEach(stage => {
                 ...
                 const canvasId = `cbrPie${stage}`;
                 const messageId = `cbrPie${stage}Message`;
        ...
        error: function(xhr, status, error) {
            ...
            $('#cbrPieInicioMessage, #cbrPieCrecimientoMessage, #cbrPieFinalizacionMessage').text('Error de conexión al cargar datos.');
        }
    });
